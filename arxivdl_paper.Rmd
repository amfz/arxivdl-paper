...
In `arxivdl` we create a package to facilitate responsibly downloading subsets of papers from arXiv.

This is useful because there 

Our package makes use of and complements the `aRxiv` package's functions for getting paper metadata from the arXiv API.
...
The main function in `arxivdl`is `download_pdf()`.
...
`clean_titles()` takes a data frame and the name of the column to clean for use as file names.
...
paper_count <- get_record_count("cs.GR", "20200101 TO 20200131")

# get metadata on all graphics papers in January 2020
metadata <- get_records("cs.GR", "20200101 TO 20200131", paper_count)
...
# get metadata for graphics papers published in January 2020
papers <- get_records("cs.GR","202001 TO 20200201", lim = 20)
